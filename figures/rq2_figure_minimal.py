...
# Create the "rq2_figures_minimal" directory if it doesn't exist
os.makedirs("rq2_figures_minimal", exist_ok=True)
...
for model in model_list:
    for benchmark in benchmark_list:
        ...
        for dim in get_dimensions(benchmark):
            ax.plot(ppq, dimension_variances[dim], linewidth=1, alpha=0.5)
        
        avg_variances = np.nanmean(list(dimension_variances.values()), axis=0)
        ax.plot(ppq, avg_variances, linewidth=3, color="red")
        ...
        plt.tight_layout()
        plt.savefig(f"rq2_figures_minimal/{model}_{benchmark}_variance_plot_minimal.png", dpi=300, bbox_inches='tight', pad_inches=0)
        plt.close(fig)
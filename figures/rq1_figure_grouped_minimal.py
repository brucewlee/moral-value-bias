...
os.makedirs("rq1_figures_grouped_minimal", exist_ok=True)
...
for benchmark in benchmark_list:
    for model in model_list:
        ...
        # Create a figure with no axis, legend, or white space
        fig = plt.figure(figsize=(len(grouping) * 1.8, 4))
        ...
        for i, interpretation in enumerate(["A", "B", "C", "D", "E", "F"]):
            ...
            bars = ax.bar(grouping.keys(), percentages, bottom=bottom, color=color, hatch='', edgecolor='black', linewidth=1)
        ...
        # Remove the axis and legend
        ax.set_xticks([])
        ax.set_yticks([])
        
        # Save the figure with no white space
        plt.savefig(f"rq1_figures_grouped_minimal/{benchmark}_{model}_grouped_percentage_minimal.png", dpi=300, bbox_inches='tight', pad_inches=0)
